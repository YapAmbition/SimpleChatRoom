...
import type { ScrMessage, ScrMonitorOptions, ScrConnectionStatus } from "./types.js";
import { scrLogin, getOrCreateSession } from "./send.js";
 ...
 */
export async function startMonitor(opts: ScrMonitorOptions): Promise<MonitorHandle> {
  ...
  const setStatus = (status: ScrConnectionStatus) => {
    if (!stopped) onStatusChange?.(status);
  };
  ...
  const authenticate = () => {
    ...
    socket.emit("login", account.botName, (resp: { ok: boolean; error?: string }) => {
      if (stopped || !socket) return;
      ...
      if (!resp.ok) {
        ...
        if (resp.error?.includes("已在线")) {
          ...
          socket!.emit("login", altName, (resp2: { ok: boolean; error?: string }) => {
            if (resp2.ok) {
              joinRoom();
            } else {
  ...
  const joinRoom = () => {
    ...
    const args: unknown[] = [account.room];
    if (account.roomPassword) args.push(account.roomPassword);

    // Callback for join-room
    const onJoined = (resp: { ok: boolean; id?: string; name?: string; error?: string }) => {
      if (stopped) return;
      if (!resp.ok) {
  ...
  socket.on("message", (msg: ScrMessage) => {
    if (stopped) return;

    // Echo suppression: ignore bot's own messages
    if (msg.user === account.botName) return;
    // Also ignore any variant name we might have logged in with
    if (msg.user.startsWith(account.botName + "-")) return;
  ...
  socket.on("history", (messages: ScrMessage[]) => {
    ...
    if (lastSeenMessageId) {
      ...
      if (idx >= 0) {
        ...
        log.debug(
          `[monitor] history: ${messages.length} total, ${newMsgs.length} new since last seen`
        );
  ...
  socket.on("presence", (data: { users: string[]; event: string; user: string }) => {
    if (stopped) return;
