...
export class MessageBatcher {
  ...
  constructor(
    ...
    private readonly onFlush: (messages: ScrMessage[]) => Promise<void>
  ) {}
  ...
  push(msg: ScrMessage): void {
    ...
    const log = getLogger();
    log.debug(
      `[batcher] queued message from ${msg.user}, queue size: ${this.queue.length}`
    );
  ...
  private async flush(): Promise<void> {
    ...
    if (this.queue.length === 0 || this.processing || this.destroyed) return;
    ...
    try {
      ...
      if (this.queue.length > 0 && !this.destroyed) {
        log.debug(
          `[batcher] ${this.queue.length} message(s) queued during processing, restarting timers`
        );
        this.resetDebounce();
        this.maxWaitTimer = setTimeout(() => {
