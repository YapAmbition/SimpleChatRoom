 ...
 */
function isMentioned(msg, account) {
 ...
 */
function formatBatchContext(messages) {
    return messages
        .map((m) => {
 ...
 */
function buildBatchedContext(messages) {
    return {
        messages,
...
export function setAgentDispatcher(dispatcher) {
 ...
 */
export async function handleBatchedInbound(messages, account) {
    const log = getLogger();
    ...
    if (account.requireMention) {
        ...
        if (mentionedMessages.length === 0) {
            log.debug(`[inbound] batch of ${messages.length} messages, none mention bot — skipping`);
            return;
        }
        log.debug(`[inbound] ${mentionedMessages.length}/${messages.length} messages mention bot`);
    ...
    const senderDesc = messages.length === 1
        ? lastSender
        : `${new Set(messages.map((m) => m.user)).size} users`;
    log.info(`[inbound] processing batch: ${messages.length} msg(s) from ${senderDesc} ` +
 ...
 */
async function dispatchViaHttpApi(body, sessionKey, account) {
    const log = getLogger();
