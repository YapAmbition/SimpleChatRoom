 ...
 */
export function listScrAccountIds(cfg) {
 ...
 */
export function resolveDefaultScrAccountId(cfg) {
 ...
 */
export function resolveScrAccount(cfg, accountId) {
    const env = envFallbacks();
    // Start with env fallbacks
    const base = {};
    if (env.serverUrl)
        base.serverUrl = env.serverUrl;
    if (env.room)
        base.room = env.room;
    if (env.botName)
        base.botName = env.botName;
    if (env.roomPassword)
        base.roomPassword = env.roomPassword;
    ...
    const validated = validateAccountConfig(base, accountId);
